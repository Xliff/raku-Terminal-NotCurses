  ...
  class Terminal::NotCurses::Main {
    ...
    method version_components (
      ...
      my int32 ($ma, $mi, $pa, $tw) = 0 xx 4;

      notcurses_version_components($ma, $mi, $pa, $tw);

      ($major, $minor, $patch, $tweak) = ($ma, $mi, $pa, $tw);
      return ($major, $minor, $patch, $tweak) if $list;
      Version.new(
        "{$major}.{$minor}.{$patch * 1000 + $tweak}"
      );
    }
