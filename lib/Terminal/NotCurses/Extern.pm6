  ...
  method nccell_bchannelexport (nccell $cl) {
    nccell_bchannelexport($cl);
  }

  method nccell_bg_alphaexport (nccell $cl) {
    nccell_bg_alphaexport($cl);
  }

  method nccell_bg_default_pexport (nccell $cl) {
    nccell_bg_default_pexport($cl);
  }

  method nccell_bg_palindex_pexport (nccell $cl) {
    nccell_bg_palindex_pexport($cl);
  }

  method nccell_bg_palindexexport (nccell $cl) {
    nccell_bg_palindexexport($cl);
  }

  method nccell_bg_rgb8export (
    nccell $cl,
    gint   $r is rw,
    gint   $g is rw,
    gint   $b is rw
  ) {
    nccell_bg_rgb8export($cl, $r, $g, $b);
  }

  method nccell_bg_rgbexport (nccell $cl) {
    nccell_bg_rgbexport($cl);
  }

  method nccell_channelsexport (nccell $c) {
    nccell_channelsexport($c);
  }

  method nccell_colsexport (nccell $c) {
    nccell_colsexport($c);
  }

  method nccell_double_wide_pexport (nccell $c) {
    nccell_double_wide_pexport($c);
  }

  method nccell_extractexport (
    ncplane  $n,
    nccell   $c,
    uint16_t $stylemask,
    uint64_t $channels
  ) {
    nccell_extractexport($n, $c, $stylemask, $channels);
  }

  method nccell_fchannelexport (nccell $cl) {
    nccell_fchannelexport($cl);
  }

  method nccell_fg_alphaexport (nccell $cl) {
    nccell_fg_alphaexport($cl);
  }

  method nccell_fg_default_pexport (nccell $cl) {
    nccell_fg_default_pexport($cl);
  }

  method nccell_fg_palindex_pexport (nccell $cl) {
    nccell_fg_palindex_pexport($cl);
  }

  method nccell_fg_palindexexport (nccell $cl) {
    nccell_fg_palindexexport($cl);
  }

  method nccell_fg_rgb8export (
    nccell $cl,
    gint   $r is rw,
    gint   $g is rw,
    gint   $b is rw
  ) {
    nccell_fg_rgb8export($cl, $r, $g, $b);
  }

  method nccell_fg_rgbexport (nccell $cl) {
    nccell_fg_rgbexport($cl);
  }

  method nccell_initexport (nccell $c) {
    nccell_initexport($c);
  }

  method nccell_load_charexport (
    ncplane $n,
    nccell  $c,
    Str     $ch
  ) {
    nccell_load_charexport($n, $c, $ch);
  }

  method nccell_load_egc32export (
    ncplane  $n,
    nccell   $c,
    uint32_t $egc
  ) {
    nccell_load_egc32export($n, $c, $egc);
  }

  method nccell_load_ucs32export (
    ncplane  $n,
    nccell   $c,
    uint32_t $u
  ) {
    nccell_load_ucs32export($n, $c, $u);
  }

  method nccell_off_stylesexport (nccell $c) {
    nccell_off_stylesexport($c);
  }

  method nccell_on_stylesexport (nccell $c) {
    nccell_on_stylesexport($c);
  }

  method nccell_primeexport (
    ncplane  $n,
    nccell   $c,
    Str      $gcluster,
    uint16_t $stylemask,
    ...
    nccell_primeexport($n, $c, $gcluster, $stylemask, $channels);
  }

  method nccell_set_bchannelexport (
    nccell   $c,
    uint32_t $channel
  ) {
    nccell_set_bchannelexport($c, $channel);
  }

  method nccell_set_bg_alphaexport (nccell $c) {
    nccell_set_bg_alphaexport($c);
  }

  method nccell_set_bg_defaultexport (nccell $c) {
    nccell_set_bg_defaultexport($c);
  }

  method nccell_set_bg_palindexexport (nccell $cl) {
    nccell_set_bg_palindexexport($cl);
  }

  method nccell_set_bg_rgb8_clippedexport (
    nccell $cl,
    gint   $r,
    gint   $g,
    gint   $b
  ) {
    nccell_set_bg_rgb8_clippedexport($cl, $r, $g, $b);
  }

  method nccell_set_bg_rgb8export (nccell $cl) {
    nccell_set_bg_rgb8export($cl);
  }

  method nccell_set_bg_rgbexport (
    nccell   $c,
    uint32_t $channel
  ) {
    nccell_set_bg_rgbexport($c, $channel);
  }

  method nccell_set_channelsexport (
    nccell   $c,
    uint64_t $channels
  ) {
    nccell_set_channelsexport($c, $channels);
  }

  method nccell_set_fchannelexport (
    nccell   $c,
    uint32_t $channel
  ) {
    nccell_set_fchannelexport($c, $channel);
  }

  method nccell_set_fg_alphaexport (nccell $c) {
    nccell_set_fg_alphaexport($c);
  }

  method nccell_set_fg_defaultexport (nccell $c) {
    nccell_set_fg_defaultexport($c);
  }

  method nccell_set_fg_palindexexport (nccell $cl) {
    nccell_set_fg_palindexexport($cl);
  }

  method nccell_set_fg_rgb8_clippedexport (
    nccell $cl,
    gint   $r,
    gint   $g,
    gint   $b
  ) {
    nccell_set_fg_rgb8_clippedexport($cl, $r, $g, $b);
  }

  method nccell_set_fg_rgb8export (nccell $cl) {
    nccell_set_fg_rgb8export($cl);
  }

  method nccell_set_fg_rgbexport (
    nccell   $c,
    uint32_t $channel
  ) {
    nccell_set_fg_rgbexport($c, $channel);
  }

  method nccell_set_stylesexport (nccell $c) {
    nccell_set_stylesexport($c);
  }

  method nccell_strdupexport (
    ncplane $n,
    nccell  $c
  ) {
    nccell_strdupexport($n, $c);
  }

  method nccell_stylesexport (nccell $c) {
    nccell_stylesexport($c);
  }

  method nccell_wide_left_pexport (nccell $c) {
    nccell_wide_left_pexport($c);
  }

  method nccell_wide_right_pexport (nccell $c) {
    nccell_wide_right_pexport($c);
  }

  method nccellcmpexport (
    ncplane $n1,
    nccell  $c1,
    ncplane $n2,
    nccell  $c2
  ) {
    nccellcmpexport($n1, $c1, $n2, $c2);
  }
