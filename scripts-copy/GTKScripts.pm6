...
# cw: New hotness
our regex        p is export { [ '*' [ \s* 'const' <!before '_'> \s* ]? ]+ }
our token        n is export { <[\w _]>+ }
our token        t is export { 'unsigned' | 'unsigned'? <n> | '(' <p> <n>? ')' \s* <parameters> }
...
our token     var is export { <t> }
our token postdec is export { (<[A..Z0..9]>+)+ %% '_' \s* [ '(' .+? ')' ]? }
...
our rule static-inline-func is export {
  ^^<macro>?\s*'static' 'inline' <func_block>
}
