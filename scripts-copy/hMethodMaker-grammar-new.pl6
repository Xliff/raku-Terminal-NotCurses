...
sub MAIN (
  ...
  $matched<top-bland><function-bland>.map( *.<func_def><true-func-def><sub>.Str ).gist.say;
  ...
  for $matched<top-bland><function-bland>[] -> $m {
    ...
    my @p;
    my $orig = ($m<func_def><true-func-def><sub> // '').Str.trim;
    my $collider = 1;

    # cw: We must handle the initialization of @tv carefully, since a type
    #     without a variable name is a valid C construct.
    my @tv =$m<func_def><true-func-def><parameters><type> [Z] $m<func_def><true-func-def><parameters><var>;
    ...
    unless @tv {
      @tv = $m<func_def><true-func-def><parameters><type>
      [Z]
      'var' «~« ^$m<func_def><true-func-def><parameters><type>.elems
    }
    ...
    my $sub = $m<func_def><true-func-def><sub>.Str.trim;
    $o_call ~= ', ...' if $m<func_def><true-func-def><parameters><va>;
    ...
    $sig ~= ', ...' if $m<func_def><true-func-def><parameters><va>;
    ...
    my $h = (
     ...
     call_params => @v.map( $ ~ *[1] ),
      call_types => @t,
         var_arg => $m<func_def><true-func-def><parameters><va>:exists
    ).Hash;
